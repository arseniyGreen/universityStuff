...
template <class T>
vector<int> findStep(vector<vector<T>>& matrix)
{
    vector<int> vers; vector<int> step;

    for(size_t i = 0; i < matrix.size(); i++){ vers.push_back(0); step.push_back(0); }

    vers[0] = 1; bool found = false;
    while(!found)
    {
        for(size_t i = 0; i < matrix.size(); i++)
        {
        ...
        found = true;
        for(size_t i = 0; i < matrix.size(); i++)
            if(vers[i] != 2) found = false;
    }
    return step;
}

template<class T>
double avgStep(vector<vector<T>>& matrix)
{
    vector<int> a = findStep(matrix);
    double av = 0;
    for(size_t i = 0; i < matrix.size(); i++) av += a[i];
    av /= matrix.size();
}
...
int main() {
    ...
    vector<int> v = findStep(m);
    for(size_t j = 0; j < mat.size(); j++) std::cout << v[j] << '\t';
    std::cout << '\n';
